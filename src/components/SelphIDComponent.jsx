...
export default function SelphIDComponent({ setMrz, setOcr, setFaceImage, setWidget }) {
	...
	async function handleExtractionFinish(event) {
		...
		if (resultMessage == "OK") {
			...
			let mrz = event.detail.detail?.result?.extractionRaw?.mrz
			let ocr = event.detail.detail?.result?.extractionRaw?.ocr
			let faceImage = event.detail.detail?.result?.images?.faceImage
